...
- [2026-05-15] (training) **LLM 코딩 에이전트의 Mock 과잉 — 외부 라이브러리 직접 mock 금지 실증 근거** — LLM 에이전트(Cursor 37% / Copilot 36% / Claude 34%) 가 인간 개발자(26%) 보다 mock 사용 비율이 10%p 이상 높음. 에이전트는 *mock 타입 95%* 에 집중하고 fake/spy 사용은 드묾. 외부 라이브러리를 직접 mock 하는 패턴이 자동 생성되어 통합 오류 감지력 손실. → 정책: 외부 라이브러리 직접 mock 금지, 자체 어댑터 인터페이스를 만든 뒤 그 인터페이스만 mock (GOOS Ch7 "Don't Mock What You Don't Own" 원칙의 실증 강화). 근거: arXiv 2602.00409 "Are Coding Agents Generating Over-Mocked Tests?" MSR 2026 (1.2M commits / 2,168 repos 분석).
- [2026-05-15] (training) **Coverage 100% 신뢰 금지 — LLM coverage-driven 테스트의 mutation kill rate 2.4%** — LLM 이 line/branch coverage 극대화 목표로 생성한 테스트의 mutation kill rate 가 TestGen-LLM 기준 2.4% 에 그침. 100% 분기 커버리지를 달성해도 mutation score 4% 인 사례 존재 (arXiv 2506.02954). Mutation-guided 피드백 (생존 mutant → LLM 재제출 → assertion 강화) 추가 시 15% 까지 상승. → coverage 합격선만으로 LLM 생성 테스트 수용 금지. REFACTOR 단계에 mutation survivor 루프 명시. 근거: arXiv 2501.12862 "Mutation-Guided LLM-based Test Generation at Meta" FSE 2025 industry paper; arXiv 2506.02954.
- [2026-05-15] (training) **LLM 생성 assertion 의 ~25% 가 false positive oracle** — 변수명·함수명이 부재하거나 임의로 붙으면 LLM 은 expected behavior 를 추론 못 하고 actual behavior 를 그대로 검증하는 mirror oracle 을 만든다. 개발자 작성 명명 규칙 사용 시 oracle 품질이 최대 16.10%p 향상. 즉 LLM 생성 oracle 은 *regression 캡처용* (현재 동작 고정) 이지 *expected behavior 검증용* 이 아님. 근거: ASE 2025 "Do LLMs Generate Useful Test Oracles?"; arXiv 2601.05542 "Understanding LLM-Driven Test Oracle Generation".
- [2026-05-15] (training) **Pact AI 생성 provider state 수동 검토 의무** — PactFlow AI 는 2026-05 기준 consumer-side HTTP 상호작용 생성만 지원. provider verification + provider state 정의 + event-based 아키텍처 = *"future updates"* 로 명시. AI 생성 Pact test 는 provider state 누락 / matcher 미사용 / 너무 엄격하거나 너무 느슨한 경향 보고. → 정책: AI 생성 consumer Pact test 는 반드시 provider state 수동 검토 후 사용, provider verification 자체는 AI 의존 금지. 근거: pactflow.io/ai (공식, 2026-05); pactflow.io/blog/ai-automation-part-2.
