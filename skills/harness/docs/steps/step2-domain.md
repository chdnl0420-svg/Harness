...
| 모드 | skill | 사용자 질의 |
|------|-------|------------|
| **noask 기본** (`/harness` 호출 시 `.harness/.noask` 존재) | `harness-plan` skill (Skill tool, skill="harness-plan") | 없음 — 한 줄 목표를 입력 카테고리의 합리적 가정으로 메우고 딥 리서치 게이트와 DDD 게이트를 거쳐 도메인 초안 작성. 모든 가정은 *"Open Questions"* 섹션에 누적. |
...
## CRITICAL: DDD 도메인 설계 게이트

step2 산출물은 구현 계획이 바로 참고할 수 있는 DDD 도메인 모델을 포함해야 한다. 이 게이트는 도메인 주도 설계(DDD)의 목표 구조를 **구현 계획의 기준으로 강제**한다. 기존 코드 구조가 DDD 모델과 다르면 step3 에서 DDD 목표 구조로 옮기는 migration plan 을 만든다.
...
검증:
- 위 항목 중 하나라도 비어 있으면 step3 진입 전에 도메인 초안을 다시 쓴다.
- 단순 CRUD 작업이어도 DDD 최소 세트(공통 업무 언어, bounded context, aggregate 또는 transaction boundary, application service, repository/adapter boundary)를 생략하지 않는다.
- DDD 용어가 기존 코드 변경으로 이어지면, 그 변경을 회피하지 말고 step3 에서 단계적 migration plan 으로 반영한다.
...
3. **DDD 게이트 확인** — 공통 업무 언어, bounded context, aggregate/invariant 판단, context map 이 포함되어야 한다. 누락 시 1번을 재호출한다.
